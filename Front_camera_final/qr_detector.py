...
class QRDetector:
    ...
    def _draw_qr_annotation(self, frame, bbox, is_selected):
        ...
        if is_selected:
            # GREEN — operator has selected this pallet
            box_color = (0, 220, 80)
            label_bg  = (0, 180, 60)
            label_text = "SELECTED"
            text_color = (255, 255, 255)
        else:
            # ORANGE — valid, tap to select
            box_color = (0, 200, 255)
            label_bg  = (0, 200, 255)
            label_text = "TAP TO SELECT"
            text_color = (0, 0, 0)
    ...
    def detect_and_filter_qrs(self, frame: np.ndarray, selected_ids: set = None) -> Tuple[List[Dict[str, Any]], int, np.ndarray]:
        ...
        selected_ids: set of pallet_id strings the operator has tapped/selected.
                      Selected QRs are drawn in GREEN; others in ORANGE.
        """
